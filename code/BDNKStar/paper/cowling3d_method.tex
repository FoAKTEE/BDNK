...
$-1\%$; three smooth barotropes agree to $\le0.08\%$. We extend the method to
...
and converges at third order to $96$--$97\%$ of the Navier--Stokes dissipation integral, while
...
Kreiss--Oliger coefficient is a $\pm0.25\%$ systematic; and time-domain frequency resolution at
affordable cost is $4\%$ of $f$, so every sub-percent figure is a parametric estimate.
\end{abstract}
...
\end{align}
evaluated by 24-point Gauss--Legendre quadrature. A face at $x = x_f$ meets the sphere in a disc
of radius $\sqrt{R^2-x_f^2}$, so $a_{x\pm} = \mathcal D(\sqrt{R^2-x_f^2};\ldots)/\Delta x^2$
directly; the volume fraction is a second Gauss quadrature of $\mathcal D$ over $x$ across the
cell, $\kappa = \Delta x^{-3}\int\mathcal D\,dx$. No interface-plane reconstruction is needed.
Quadrature returns $1\pm\mathrm{few}\,\epsilon$ for a full cell, so all seven fields are clamped to
$[0,1]$. Verified against the analytic sphere: $\sum\kappa\,\Delta x^3 = \tfrac43\pi R^3$ to
$10^{-3}$ at $N=32$ and the closure condition---the face-aperture area vectors balancing the
boundary face---reproduces $4\pi R^2$ at second order. Cells with $\kappa$ below a floor
...
The $\ell=2$, $m=0$ seed is $\de = A\,\varepsilon_0\,(r/R)\,(2z^2-x^2-y^2)/r^2$ with
$A = 10^{-3}$ (\BDNK: a radial-velocity seed). The diagnostic is the $\sg$-weighted
quadrupole $q_{20}(t) = \sum\de\,(2n_z^2-n_x^2-n_y^2)\,\sg\,\Delta x^3$ over interior cells, and
the central $\de$. Because the code is linear, amplitude proportionality is exact to
$10^{-14}$ (verified over $\times100$ and $\times3000$), which makes the linear engine the
control for any nonlinear-engine claim (Sec.~\ref{sec:dg}).
...
Richardson extrapolation gives $2.51$--$2.54\times10^{-3}\,\Msun^{-1}$, which is $96$--$97\%$ of
the Newtonian-form Navier--Stokes dissipation integral $0.08746\,\hat\eta = 2.6238\times10^{-3}$
for this star---a few-percent causal suppression in the expected direction. The frame spread of
...
shift relative to the inviscid control ($+0.40/+0.28/+0.15\%$ at $N=24/32/40$) is \emph{not}
converged and is quoted only as $<0.2\%$.
...
grid's genuine degeneracy violation at $9\times10^{-5}$. Two facts, however, make the linear
cut-cell method the one to use for precision. The DG background is not static: the well-balanced
...
deformability. The engine is linear and seeds $\ell=2$, $m=0$ only; extending the seed and the
diagnostic to $\mathrm{Re}\,Y_{22}$ on the full cube is a small change and the natural next step,
and would give an $m$-degeneracy test free of the DG engine's erosion.

\emph{Numerical.} Time-domain resolution at affordable cost is $\Delta f/f = 4.4\%$; $\KO$ is a
$\pm0.25\%$ systematic; the cut-cell path develops a late-time growing surface mode at
