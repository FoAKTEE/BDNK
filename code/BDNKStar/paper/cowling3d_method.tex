...
Figure~\ref{fig:ts} shows what the engine produces: the ideal run itself decays, at the numerical
floor, and the viscous run decays faster; the difference is the physics.

\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig1_timeseries}
\caption{The $\ell=2$ quadrupole moment $q_{20}(t)$ of the benchmark star at $N=24$, $\KO=0.005$,
for the ideal engine and for \BDNK\ in frame $F_1$ at $\hat\eta = 0.03$, each normalised to its
seed. Dashed: exponential fits to the positive peaks. The ideal run decays at the numerical floor
$\gamma_0$ (Kreiss--Oliger dissipation and the surface); the viscous damping is the difference.
The first half-period is the seed relaxing onto the mode.}
\label{fig:ts}
\end{figure}
...
Table~\ref{tab:anchor} and Fig.~\ref{fig:conv} are the central validation. With cut cells at
$N=48$ the $f$-mode is $-0.083\%$ from the shooting value and $p_1$ is $-0.98\%$; the masked
engine at $\KO = 0.004$ is $-6.93\%$ and $-6.95\%$ at $N=32$ and $N=40$, i.e.\ the error does not
converge in this range and would be reported as a healthy result by any pipeline that did not
compare to a reference. Raising the mask's dissipation to $\KO = 0.01$ does not repair it but
hides it: the peak becomes ill-defined, the two estimators disagreeing by $1$--$2\%$
(Fig.~\ref{fig:conv}, open symbols), so the masked engine's answer depends on $\KO$ at the
seven-percent level while the cut-cell answer moves by $\pm0.25\%$.
...
\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig2_convergence}
\caption{$\ell=2$ $f$-mode of the benchmark star against the shooting solver ($1.88291\kHz$),
as a function of resolution. Filled circles: cut cells, $\KO=0.01$. Filled squares: binary mask
(rigid wall), $\KO=0.004$. Open squares: the mask at $\KO=0.01$, where the peak is ill-defined.
Symbols are the mean of the periodogram and matrix-pencil estimates; error bars are half their
difference. Cut-cell and $\KO=0.01$ mask records are nine periods; the $\KO=0.004$ mask records
are 23 periods.}
\label{fig:conv}
\end{figure}
...
stencil made visible as a mode splitting. It decreases with resolution---$1.12$, $0.40$ and
$0.32\%$ at $N = 24, 32, 40$ (Fig.~\ref{fig:msplit})---consistent with $O(h^2)$ within the
$\pm0.1\%$ spread of the two estimators on nine-period records, while the $E_g$ splitting stays
below $10^{-12}\%$ at every $N$. It is a cleaner measurement of the grid's anisotropy than the
nonlinear engine could give (Sec.~\ref{sec:dg}).

\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig3_msplit}
\caption{Splitting of the $\ell=2$ $f$-mode by the Cartesian grid. The $T_{2g}$ member
$\mathrm{Re}\,Y_{21}$ is split from the $E_g$ pair by the cubic anisotropy of the stencil; the
dotted line is $h^2$ scaling from $N=24$. The $E_g$ partners $Y_{20}$ and $\mathrm{Re}\,Y_{22}$ are
degenerate to round-off at every resolution.}
\label{fig:msplit}
\end{figure}
...
\label{sec:eos}
\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig4_surface_index}
\caption{Growth or decay of the $f$-mode record over 23 periods at $N=48$, against the surface
index $n$ of each equation of state ($\varepsilon\propto(R-r)^n$ near $r=R$). Green: the three
analytic polytropes, decaying. Red: the four crusted Read~\emph{et al.} EOS, growing by
$10^{11}$--$10^{15}$. The dashed line is neutral stability.}
\label{fig:surface}
\end{figure}
...
in the $N=48$ end/start ratio, monotone over nineteen decades with no exception
(Fig.~\ref{fig:surface}). The SLy crust's
...
integral $0.08746\,\hat\eta = 2.6238\times10^{-3}$ for this star (Fig.~\ref{fig:damping}). The
apparent few-percent ``causal suppression'' of the $\KO=0.01$ sequence was Kreiss--Oliger bias. The frame spread of
...
\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig5_damping}
\caption{Viscous damping rate of the $f$-mode at $\hat\eta=0.03$ from the difference protocol,
against resolution. The three Clarisse frames at $\KO=0.01$ lie on one curve (spread
$0.045\%$ at $N=40$). At $\KO=0.005$ (frame $F_1$, matrix pencil) the rate approaches the
Navier--Stokes dissipation integral for this star (dashed).}
\label{fig:damping}
\end{figure}
...
observations settled it (33 runs; \texttt{repro/bdnk3d\_viscous\_shift.jl}; Fig.~\ref{fig:shift}).
First, with the
...
\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig6_shift}
\caption{Viscous frequency shift of the $f$-mode in frame $F_1$. Top: against resolution at
$\hat\eta=0.03$, for two Kreiss--Oliger coefficients, with the damped-oscillator prediction
$-(\gamma^2-\gamma_0^2)/2\omega^2$ built from the measured damping rates (crosses). Bottom:
against $\hat\eta$ at $N=40$, $\KO=0.005$; the dotted curve is $\propto\hat\eta^2$. Symbols are the
mean of the two estimators, error bars half their difference.}
\label{fig:shift}
\end{figure}
...
$\hat\eta$ axis at $N=24$ ($1/h = 1.043$; Fig.~\ref{fig:stab}): six predictions, six correct in sign, with growth
...
\begin{figure}[tb]
\includegraphics[width=\columnwidth]{figs/fig7_stability}
\caption{Measured growth rate of $\max|\delta v|$ against viscosity at $N=24$ for frames $F_1$
(circles) and $F_3$ (triangles), with the frame times built from each run's own $\hat\eta$. Dashed
lines: the predicted onset $\hat\eta_c = C\,h$ from the critical-wavenumber criterion. $F_3$
destabilises first although it violates the stability bound in fewer cells.}
\label{fig:stab}
\end{figure}
...
The DG engine also supplied a lesson about what a standard RKDG scheme does to a star. In its
first form the background was not static: the well-balanced subtraction of the equilibrium
residual is exact, but the mean-based Zhang--Shu positivity limiter is not. It scales every field
toward its cell mean, $U \leftarrow \bar U + \theta(U-\bar U)$, and in a surface element, where
$D_{\rm eq}$ spans orders of magnitude, any $\theta<1$ flattens the equilibrium's own profile and
pushes mass from the stellar edge into the atmosphere nodes on every stage of every step. With the
limiter off the unseeded star is static to $10^{-12}$; with it on, the same unseeded star is
thrown into a $\pm3\%$ radial oscillation with $0.6c$ surface velocities within $50\,\Msun$, and
once seeded $\rho_c$ erodes $-10\%$ by $4.6$ periods and $-31$ to $-43\%$ by $3000\,\Msun$, which
put its absolute $f$ at $-16\%$ and capped the usable record at $\Delta f/f = 21.6\%$. The
Rusanov dissipation contributes a second, smaller channel: it acts on the equilibrium jump across
each face with a state-dependent wave speed. Both are removed by working on the deviation
$\delta U = U - U_{\rm eq}$. The limiter scales toward a reference $R$ that equals $U_{\rm eq}$
when $\delta U$ vanishes and has the cell mean of $U$---the mean deviation is shared out in
proportion to the background, so the atmosphere nodes receive essentially none of it---and whose
energy carries the cold kinetic energy $\sqrt{R_D^2+|R_S|^2}-R_D$ of its own momentum; without
that term the reference has negative pressure at the outermost stellar nodes for a $1\%$ velocity
seed, because $\tau_{\rm eq}\propto\rho^2$ vanishes at the surface faster than $\tfrac12 D\bar v^2$,
and the surface elements fall back to the flattening limiter. The flux dissipates the
state-dependent wave speed on $\delta U$ only and a frozen equilibrium sound speed on the
equilibrium jump. The equilibrium is then a bitwise fixed point of limiter and right-hand side:
the unseeded star stays static to $2\times10^{-13}$ in $\rho_c$ with the limiter on, and a $1\%$
velocity seed leaves $\rho_c$ at $-7\times10^{-4}$ after nine periods, decelerating toward the
value set by the thermalised mode energy, with no stellar element ever handed to the fallback.
The $f$-mode of the fixed engine is $1.69$--$1.85\kHz$ at $K=6$--$10$ in $4.6$--$12$-period
windows ($-2$ to $-10\%$, with estimator scatter of the same size and no monotone trend in $K$)
with a decaying envelope, where the eroding engine gave $1.57$--$1.61\kHz$ with a growing one.
Two thirds of that offset turned out to be an incomplete source term (the metric-derivative
term $\tfrac{\alpha}{2}\sqrt\gamma\,T^{ab}\partial_i\gamma_{ab}$ and the $\sqrt\gamma$ factor of
the gravity term were missing, so the static balance was not TOV; the well-balanced subtraction
had hidden it); with the complete sources the same runs give $f$ at $-0.5$, $-1.1$ and $-3.0\%$
for $K=6$, $8$ and $10$. What remains is the surface systematic of a nodal DG star at $14$--$23$
nodes across $R$, the analogue of the $-7\%$ that the linear engine shows with a masked surface
(Sec.~\ref{sec:cutcell}), and it is not a limiter effect.

Two facts still make the linear cut-cell method the one to use for precision. The octant's three
separate reflections admit only the $E_g$ doublet $\{Y_{20},\mathrm{Re}\,Y_{22}\}$; the entire
$T_{2g}$ triplet, including all of $m=1$, is structurally absent from the domain. And a
time-domain frequency from a nonlinear evolution carries the $1/T$ resolution of its record,
which at $K\le10$ is tens of percent. A search for nonlinear amplitude dependence in the pre-fix
...
exact amplitude proportionality was the control that established this; with a static background
the experiment can now be repeated at the resolution it needs.
