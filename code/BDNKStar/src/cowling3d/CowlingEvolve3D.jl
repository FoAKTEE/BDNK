...
# Verified against the analytic sphere: volume to ~1e-7 relative, and the closure
# condition (face-aperture area vectors balancing the boundary face) reproduces
# 4 pi R^2 at 2nd order.
function _disc_rect(a, y0,y1, z0,z1, gx, gw)
    a <= 0 && return 0.0
    lo=max(y0,-a); hi=min(y1,a); hi<=lo && return 0.0
    c=(hi+lo)/2; hh=(hi-lo)/2; sacc=0.0
    @inbounds for i in eachindex(gx)
        y=c+hh*gx[i]; t=a*a-y*y; t<=0 && continue
        zh=sqrt(t); sacc += gw[i]*max(0.0, min(z1,zh)-max(z0,-zh))
    end
    sacc*hh
end
function _gauss(ng)
...
function _cutcell_geometry(s::Star3D)
    ...
    gx,gw=_gauss(24); h=dx/2
    nq=24; qx,qw=_gauss(nq)
    @inbounds for k in 1:N, j in 1:N, i in 1:N
        ...
        ax(xf)= _disc_rect(sqrt(max(0.0,R^2-xf^2)), cy-h,cy+h, cz-h,cz+h, gx,gw)/dx^2
        ay(yf)= _disc_rect(sqrt(max(0.0,R^2-yf^2)), cx-h,cx+h, cz-h,cz+h, gx,gw)/dx^2
        az(zf)= _disc_rect(sqrt(max(0.0,R^2-zf^2)), cx-h,cx+h, cy-h,cy+h, gx,gw)/dx^2
        axm[i,j,k]=ax(cx-h); axp[i,j,k]=ax(cx+h)
        aym[i,j,k]=ay(cy-h); ayp[i,j,k]=ay(cy+h)
        azm[i,j,k]=az(cz-h); azp[i,j,k]=az(cz+h)
        acc=0.0
        for q in eachindex(qx)
            xq=cx+h*qx[q]
            acc += qw[q]*h*_disc_rect(sqrt(max(0.0,R^2-xq^2)), cy-h,cy+h, cz-h,cz+h, gx,gw)
        end
        κ[i,j,k]=acc/dx^3
    end
...
"""
    setup_evo3d(s; σ_ko=0.02, η̂=0.0, ζ̂=0.0)
...
surface). η̂=ζ̂=0 ⇒ the ideal (Phase-2) engine, unchanged.
"""
function setup_evo3d(s::Star3D; σ_ko::Float64=0.02, η̂::Float64=0.0, ζ̂::Float64=0.0,
                     cut::Bool=false, κmin::Float64=0.5)
    N = s.grid.N
    w0 = s.ε0 .+ s.p0
