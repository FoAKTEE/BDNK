...
| `test_dyngr` | **1+1D DYNAMICAL GR — Stage 2** (`dyngr/DynGR1D.jl`): the **first non-Cowling engine** — radial Valencia hydro + constrained metric (Hamiltonian constraint + polar slicing) **re-solved each step from the matter**. Metric genuinely evolves (freeze→Δ=0 exactly, dynamical→moves; decisive); TOV stationary (drift 2.5e-11, no cheat); **collapse-to-BH of the UNSTABLE-branch star** (Font et al. 2002 ρ_c=7.993e-3: α_c→0.001, 2m/r→0.96, M_AH=1.273; controls: the stable star takes the same kick without collapsing, and the same star kicked outward migrates to the stable branch). Radial mode **independently validated**: after the well-balancing fix of VALIDATION §7.15 the dynamical engine (2.1304 kHz) matches the corrected Chandrasekhar SL eigensolver (2.1236 kHz) to **+0.32%** and the frozen-metric run matches the Cowling eigensolver to −0.26% (before the fix: 2.02 kHz, −4.9%), and the published Kokkotas–Ruoff n=1 κ=100 polytrope band (2.1–2.3 kHz, our exact EOS family); sign-of-shift resolved (ω²_GR<ω²_Cowling, GR softens — the prior eigensolver had a P,Q↔W exponent swap, now fixed). F²→0 at M_max behavioral. Figs `dyngr_1d.png`, `dyngr_radial_validation.png` |
