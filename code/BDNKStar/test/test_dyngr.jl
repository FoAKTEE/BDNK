...
@testset "DynGR1D — dynamical-GR 1+1D engine (Stage 2)" begin
    ...
    @testset "metric responds: dynamical radial mode below frozen" begin
        ...
        Ffrozen = fmode(true,  (3.5, 6.5))      # frozen-metric control (≈ Cowling)
        ...
        @test 1.4 < Fdyn < 2.6                  # dynamical fundamental ~2 kHz
        @test 4.0 < Ffrozen < 5.4               # frozen ~4.6 kHz (≈ Cowling 4.0)
        @test Fdyn < 0.7*Ffrozen                # metric response LOWERS the mode
        @test isapprox(Ffrozen, Fcow; rtol=0.25)# frozen tracks validated Cowling
        # GATE: dynamical engine matches the corrected full-GR eigenvalue to the
        # achieved tolerance (engine 2.03 vs SL 2.124 kHz ≈ 4.6% low; periodogram/
        # grid resolution). The validated routes (SL+shooting) both give 2.124 kHz.
        @test isapprox(Fdyn, Fgr; rtol=0.10)
    end
    ...
    @testset "radial-pulsation eigensolvers" begin
        ...
        # the dynamical engine (F_dyn≈2.0 < F_frozen≈4.6 kHz).
        @test ω2gr[1]  < ω2cow[1]
    ...
    @testset "collapse: lapse collapse + apparent horizon" begin
        εc_c = 0.003                              # moderately compact (2M/R≈0.48)
        starc = solve_tov(eos, εc_c; h=2e-4)
        eng, st = setup_dyngr(eos, εc_c; N=500, cfl=0.3, rmax_fac=1.5)
        seed_dyngr_velocity!(st, eng; A=-0.03, profile=:linear)   # inward kick
        ...
        # control: stable star, same kick magnitude, does NOT collapse
        eng2, st2 = setup_dyngr(eos, εc; N=500, cfl=0.3, rmax_fac=1.5)
        seed_dyngr_velocity!(st2, eng2; A=-0.005, profile=:linear)
        res2 = evolve_dyngr!(st2, eng2; tmax=40*star.R, probe_frac=0.5)
        @test !res2.collapsed
